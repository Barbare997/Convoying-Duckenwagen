...
class GridSpacingController(object):
    ...
    __slots__ = (
        "_last_span", "_last_ts", "_span_dot", "_v_leader",
        "_span_target", "_span_too_close",
    )
    ...
    def reset(self):
        ...
        self._span_dot = 0.0
        self._v_leader = 0.0
        self._span_target = 32.0
        self._span_too_close = 44.0
    ...
    @property
    def span_target_px(self):
        return self._span_target

    @property
    def span_too_close_px(self):
        return self._span_too_close

    def observe(self, span_px, ts, cfg, current_cmd, span_target=None, span_too_close=None):
        """Ingest a leader span sample (grid dot spacing or detector bbox px)."""
        span = float(span_px)
        ...
        self._span_target = (
            ...
            else float(cfg.get("span_target_px", 32.0))
        )
        self._span_too_close = (
            float(span_too_close)
            if span_too_close is not None
            else float(cfg.get("span_too_close_px", 44.0))
        )
        span_target = self._span_target
    ...
    def compute_target_speed(
        self, cfg, min_speed, max_speed, span_target=None, span_too_close=None,
    ):
        """Same spacing law in cruise, turn, and recovery."""
        if self._last_span is None:
            return max(min_speed, min(max_speed, float(cfg.get("slow_speed", 0.16))))

        st = float(span_target) if span_target is not None else self._span_target
        tc = float(span_too_close) if span_too_close is not None else self._span_too_close
        kp = float(cfg.get("spacing_kp", 0.010))
        kd = float(cfg.get("spacing_kd", 0.018))
        error = st - self._last_span
        v = self._v_leader + kp * error - kd * self._span_dot
        v = max(min_speed, min(max_speed, v))

        if self._last_span >= tc:
            v = min(v, float(cfg.get("span_too_close_speed", 0.05)))
